...
function updateStats(requests) {
  const stats = {
    ...
    pending: requests.filter((r) => r.status_hr === "pending").length,
    approved: requests.filter((r) => r.status_hr === "approved").length,
    rejected: requests.filter((r) => r.status_hr === "rejected").length,
  };
...
function displayRequests(requests, filter = "all") {
  ...
  const filteredRequests =
    filter === "all"
      ? requests
      : requests.filter((r) => r.status_hr === filter);

  filteredRequests.forEach((request) => {
    if (!request.status_hr) {
      console.error("Request missing status_hr:", request);
      return;
    }

    // Determine if HR buttons should be disabled
    const isHODApproved = request.status_hod === "approved";
    const isHRPending = request.status_hr === "pending";
    // New condition: disable HR actions if item is Printer and ED approval is not done
    const isEDApproved = request.status_ed === "approved";
  ...
  tr.innerHTML = `
          ...
          <td>
              ${
                request.status_hr === "pending" && !request.actionedViaEmail?.hr
                  ? `
...
async function handleAction(e) {
  ...
  let requestId = button.dataset.id;
  console.log("handleAction: initial requestId from button:", requestId);
  const role = button.dataset.role;
  const action = button.classList.contains("approve") ? "approved" : "rejected";

  // Fix for undefined requestId: try to get from closest tr data attribute
  if (!requestId) {
    const tr = button.closest("tr");
    if (tr) {
      requestId = tr.querySelector(".request-id-link")?.getAttribute('data-id');
      console.log("handleAction: fallback requestId from tr:", requestId);
    }
  }
...
function initDashboard() {
  ...
  document.getElementById("requestsTableBody").addEventListener("click", function (e) {
    ...
    console.log("Closest action-btn:", btn);
    if (btn) {
      e.preventDefault();
      handleAction(e);
      return;
