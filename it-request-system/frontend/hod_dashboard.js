...
async function loadRequests() {
  ...
  try {
    ...
    if (!Array.isArray(allRequests)) {
      console.error("Expected an array of requests but got:", allRequests);
...
function updateStats(requests) {
  const stats = {
    ...
    pending: requests.filter((r) => r.status_hod === "pending").length,
    approved: requests.filter((r) => r.status_hod === "approved").length,
    rejected: requests.filter((r) => r.status_hod === "rejected").length,
  };
...
function displayRequests(requests, filter = "all") {
  ...
  const filteredRequests =
    ...
    filteredRequests.forEach((request) => {
      if (!request[`status_hod`]) {
        console.error("Request missing status_hod:", request);
        return;
      }
      ...
      tr.innerHTML = `
              <td>
                <a href="#" class="request-id-link" data-id="${request.id !== undefined ? request.id : request._id}">
                  #${request.id !== undefined ? request.id : (request._id ? request._id.slice(-4) : "")}
                </a>
              </td>
            <td>${request.name || ""}</td>
            ...
            <td>
                  ${
                    request[`status_hod`] === "pending" && !request.actionedViaEmail?.hod
                      ? `
                      <button class="btn btn-sm btn-outline-success action-btn approve" data-id="${request.id !== undefined ? request.id : request._id}" data-role="hod">
                          <i class="fas fa-check"></i>
                      </button>
                      <button class="btn btn-sm btn-outline-danger action-btn reject" data-id="${request.id !== undefined ? request.id : request._id}" data-role="hod">
...
async function handleAction(e) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      alert("Failed to update request status: " + errorText);
      throw new Error("Failed to update request status: " + errorText);
    }
